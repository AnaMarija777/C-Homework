...
List<Car> europeOrigin1 = CarsData.Cars.Where(x => x.Origin == "Europe").ToList(); 
...
List<Car> uniqueCyl = CarsData.Cars.DistinctBy(x => x.Cylinders).OrderByDescending(y => y.Cylinders).ToList(); 
...
Car fastestCar1 = CarsData.Cars.OrderByDescending(x => x.HorsePower).First();
...
List<Car> fastestCars1 = CarsData.Cars.OrderByDescending(x => x.HorsePower).Take(5).ToList(); 
...
List<string> uniqOrig = CarsData.Cars.OrderBy(x => x.Origin).Select(y => y.Origin).ToList();
...
List<Car> moreThancyl = moreThanCertainCyl.OrderBy(x => x.HorsePower).ToList(); 
